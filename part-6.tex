...
\section{Stochastic gradient descent}
  ...
  \begin{proof}
    ...
    the inside expectation gives a function of $\omega_{k}$ so that this term equals to \marginnote{$\omega_{k}$ is an element of
      $\mathcal{H}_{k}$ and $\mathbb{E}[f(Y)|Y]=f(Y)$} $\mathbb{E}[\nabla_{\omega}f(\omega_{k},X)]$. Therefore,
    \begin{equation*}
