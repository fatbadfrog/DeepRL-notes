...
\item
  ...
  \begin{proof}{\textbf{(b)} $E[E[X|Y,Z]]=E[X]$}
    \\ \\$E[X|Y,Z]$ is a funtion of $Y,Z$, so we have
    \begin{align*}
      E[E[X|Y,Z]] &= E[f(Y,Z)] \\
      ...
      &= E[X]
    \end{align*}
    From the proof, we can see that the number of conditional random variables ($Y,Z$) can be expand to any finite
    number. So we have $E[E[X|Y_{1},\cdots,Y_{n}]]=E[X]$.
  \end{proof}
  \begin{proof}{\textbf{(c)} E[E[X|Y]|Y]=E[X|Y]}
    This proof follows immediately from property Lemma \ref{le2}(e). Because $E[X|Y]$ is a function of Y whihch is also
